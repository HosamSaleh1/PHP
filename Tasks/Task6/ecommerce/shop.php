...
if($_GET){
    ...
    if(isset($_GET['sub'])){
        if(is_numeric($_GET['sub'])){
            ...
            $subCategoryData->setStatus(1);
            $checkReuslt = $subCategoryData->checkIfSubExists();
            if($checkReuslt){
                $productData->setSubcategory_id($_GET['sub']);
                $productsResult = $productData->getProductsBySub();
            }else{
        ...
        if(is_numeric($_GET['cat'])){
            ...
            $categoryData->setStatus(1);
            $checkReuslt = $categoryData->checkIfCatExists();
            if($checkReuslt){
                $productData->setCategory_id($_GET['cat']);
                $productsResult = $productData->getProductsByCat();
            }else{
                header('location:errors/404.php');die;
            }
        ...
        if(is_numeric($_GET['brand'])){
            ...
            $brandData->setStatus(1);
            $checkReuslt = $brandData->checkIfBrandExists();
            if($checkReuslt){
                $productData->setBrand_id($_GET['brand']);
                $productsResult = $productData->getProductsByBrand();
            }else{
                header('location:errors/404.php');die;
            }
    ...
    $productsResult = $productData->read();
}
if ($productsResult) {
    $products = $productsResult->fetch_all(MYSQLI_ASSOC);
} else {
    $message = "<div class='alert alert-warning'> No Products Yet </div>";
}
?>
                    ...
                    <div class="product-grid product-view pb-20">
                        <div class="row">
                            ...
                            ?>
                                ...
                                ?>
                                <div class="col-12 alert alert-warning text-center"> <?= $message ?> </div>
                            <?php
                            }
                    ...
                    <div class="shop-widget">
                        ...
                        <div class="shop-catigory">
                            <ul id="faq">
                                ...
                                $subCategoryData->setStatus(1);
                                $categoryData->setStatus(1);
                                $categoriesResult = $categoryData->read();
                                if ($categoriesResult) {
                                ...
                                ?>
                                        <li> <a data-toggle="collapse" data-parent="#faq" href="#shop-catigory-<?= $index ?>"><?= $category['name_en'] ?> <i class="ion-ios-arrow-down"></i></a>
                                            <ul id="shop-catigory-<?= $index ?>" class="panel-collapse collapse ">
                                                ...
                                                $subcategoriesResult = $subCategoryData->getSubsFromCats();
                                                if ($subcategoriesResult) {
                                                    $subcateogries = $subcategoriesResult->fetch_all(MYSQLI_ASSOC);
                                                    foreach ($subcateogries as $index => $sub) {
                                                ?>
                                                        <li><a href="shop.php?sub=<?= $sub['id'] ?>"><?= $sub['name_en'] ?></a></li>
                                                <?php
                                                    }
                                                } else {
                                                    $noSubcategories = "<div class='alert alert-warning'> No SubCateogories Yet </div>";
                                                }
                                                ?>
                                ...
                                <?php
                                    ...
                                    $noCategories = "<div class='alert alert-warning'> No Cateogories Yet </div>";
                                }
                                ?>
